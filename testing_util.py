...
def read_image_pairs_by_distance(base_folder, method="SGBM"):
    ...
    for subdir, dirs, files in os.walk(base_folder):
        ...
        if subdir != base_folder:
            ...
            if method == "realsense":
                left_images = sorted([f for f in files if 'original' in f])
                right_images = sorted([f for f in files if 'depth' in f])

                # Empareja las imágenes por su timestamp
                for left_img in left_images:
                    timestamp = left_img.split('_original')[0]
                    corresponding_right_img = timestamp + '_depth.png'

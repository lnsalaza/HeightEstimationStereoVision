...
def process_situation(situation, variations, configs, camera_type, method_used, use_roi, mask_type, normalize):
    ...
    for variation, letter in zip(variations, alphabet):
        ...
        if method_used in ["RAFT", "SELECTIVE", "WLS-SGBM", "SGBM"]:
            ...
            img_left = cv2.imread(variation[0])
            img_right = cv2.imread(variation[1], cv2.IMREAD_UNCHANGED)
        
        # Generar y procesar nubes de puntos filtradas individuales
        point_cloud_list, colors_list, keypoints3d_list, max_coords = generate_individual_filtered_point_clouds(
...
def process_all_situations(pairs, configs, camera_type, method_used, is_roi, mask_type, normalize):
    all_data = []
    base_filename = f"./datasets/data2025/estable/NORM-DEPTH-MULTIPLE-PEOPLE/NORM-{method_used}-DEPTH-3-MULTIPLE-PEOPLE"
...
def process_all_situations_path(root_folder, configs, camera_type, method_used, is_roi, mask_type, normalize):
    all_data = []
    base_filename = f"./datasets/data2025/estable/EXTRA/NORM-{method_used}_EXTRA"
                    ...
                    for file in os.listdir(height_path):
                        if method_used in ["RAFT", "SELECTIVE", "WLS-SGBM", "SGBM"]:
                            ...
                            if file.endswith('original.jpg'):
                                right_file = file.replace('original.jpg', 'depth.png')
                                if right_file in os.listdir(height_path):
                                    left_path = os.path.join(height_path, file)
                                    right_path = os.path.join(height_path, right_file)
                                    image_pairs.append((left_path, right_path))
...
def process_situation_path(situation_depth, situation_height, image_pairs, configs, camera_type, method_used, use_roi, mask_type, normalize):
    ...
    for index, (left_path, right_path) in enumerate(image_pairs):
        ...
        point_cloud_list, colors_list, keypoints3d_list, _ = generate_individual_filtered_point_clouds(
            img_left, img_right, configs, method_used, use_roi, use_max_disparity=True, normalize=normalize
        )
...
if __name__ == "__main__":
    ...
    configs = load_config("profiles/DualLense_no_rotation.json")  # Tu configuración específica
    ...
    methods = ["WLS-SGBM", "RAFT", "SELECTIVE", "SGBM", "realsense"]

    
    # for method_used in methods:
        # pairs = read_image_pairs_by_distance("../PC_2025/pc/Stereo/altura_variable") #Tu diccionario de situaciones y variaciones
        # if method_used == "realsense":
            # pairs = read_image_pairs_by_distance("../PC_2025/pc/Intel/altura_variable", method="realsense")
    ...
    for method_used in methods:
        if method_used == "realsense":
            process_all_situations_path("../PC_2025/pc/Intel/altura_variable", configs, method_used, method_used, is_roi, mask_type, apply_correction)
        else:
            process_all_situations_path("../PC_2025/pc/Stereo/altura_variable", configs, method_used, method_used, is_roi, mask_type, apply_correction)

...
def get_RAFT_disparity_map(
        img_left_array, img_right_array, restore_ckpt, 
        output_directory="raft_demo_output", save_numpy=False, mixed_precision=False, valid_iters=32, 
    ...
    with torch.no_grad():
        ...
        for (image1_array, image2_array) in tqdm(list(zip(left_images, right_images))):
            ...
            disparity_maps.append(flow_up.cpu().numpy().squeeze())
